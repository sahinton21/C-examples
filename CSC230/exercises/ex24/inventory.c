...
int itemComp( void const *pA, void const *pB )
{
  ...
  //We are going to compare in meters so convert the imperial to metric
  double lenA = 0;
  if(itemA->mtype == imperial){
    double frac = (double)itemA->length.ival.numerator / (double)itemA->length.ival.denominator;
    lenA = (double)itemA->length.ival.inches + frac;
    lenA *= INCHES_TO_METERS; 
  }
  else{
  ...
  if(itemB->mtype == imperial){
    double frac = (double)itemB->length.ival.numerator / (double)itemB->length.ival.denominator;
    lenB = (double)itemB->length.ival.inches + frac;
    lenB *= INCHES_TO_METERS; 
  }
  else{
...
int main( int argc, char *argv[] )
{
  ...
  for ( int i = 0; i < icount; i++ ) {
    ...
    //Read the id, then type.
    char type = 0;
    fscanf(fp, "%d %c", &ilist[i].sku, &type);

    //If it is an m read a double otherwise read all the values for an imperial measurement
    if(type == 'm'){
      ...
      //Might need to adjust imperial with a zero numerator
      if(ilist[i].length.ival.numerator == 0){
        ilist[i].length.ival.denominator = 0;
      }
    }
  }
