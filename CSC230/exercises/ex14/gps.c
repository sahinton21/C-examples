...
void readCity( FILE *fp, struct City *cty ) {
  ...
  while(current != EOF && current != '\n'){
    ...
    if(current >= '0' && current <= '9')
    {
      ...
      cty->longitude = longitude;
      current = fgetc(fp);
    } 
    else if(current != ' ')
    {
      cty->name[nameI] = current;
      nameI++;
...
int main( int argc, char *argv[] )
{
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    //Iterate Through every city finding the shortest distance
    double shortest = EARTH_RADIUS;
    int shortestIndex = 0;
    for(int i = 0; i < n; i++){
      double currentDist = globalDistance(lat, lon, cities[i].latitude, cities[i].longitude);
    ...
    printf("%s ", cities[shortestIndex].name);
    printf("%.2lf\n",shortest);
