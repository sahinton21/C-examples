  ...
  while ( fmt[ pos ] ) {
    if ( fmt[ pos ] == '%' ) {
      ...
      if(fmt[pos + 1] == 's'){
        ...
        //Now get each digit into the array of characters and flip them afterwards
        //Special case of just a zero
        int start = len;

        //Put the first character always and catches a zero case.
        append(&dest, &len, &cap, (num % 10) + '0');
